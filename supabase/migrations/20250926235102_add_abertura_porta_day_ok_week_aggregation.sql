-- Drop the old hourly-only RPC function
DROP FUNCTION IF EXISTS get_abertura_porta_hourly_aggregation(INTEGER, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, INTEGER[], INTEGER);

-- Create unified RPC function for door opening aggregation (hourly and day-of-week)
CREATE OR REPLACE FUNCTION get_abertura_porta_aggregation(
...
BEGIN
    ...
    ELSE
        -- Day of week aggregation (0-6, where 0=Sunday, 1=Monday, etc.)
        RETURN QUERY
        SELECT 
