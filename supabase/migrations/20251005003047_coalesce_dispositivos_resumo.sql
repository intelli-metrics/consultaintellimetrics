-- Fix for "cannot accumulate empty arrays" error in get_lista_dispositivos_resumo function
-- The issue occurs when subqueries return empty results and PostgreSQL has issues with aggregation functions
...
BEGIN
    ...
    FROM
        ...
        LEFT JOIN (
            SELECT 
                sr."cdDispositivo",
                COALESCE(SUM(sr."nrValor"), 0) AS "nrPorta"
            FROM 
        ...
        LEFT JOIN (
            SELECT 
                sr."cdDispositivo",
                COALESCE(SUM(sr."nrValor"), 0) AS "nrPessoas"
            FROM 
                "public"."TbSensorRegistro" sr
                JOIN "public"."TbSensor" s ON sr."cdSensor" = s."cdSensor"
            WHERE 
        ...
        LEFT JOIN (
            SELECT 
                sr."cdDispositivo",
                COALESCE(AVG(sr."nrValor"), 0) AS "nrTemp"
            FROM 
                "public"."TbSensorRegistro" sr
                JOIN "public"."TbSensor" s ON sr."cdSensor" = s."cdSensor"
            WHERE 
        ...
        LEFT JOIN (
            SELECT 
                ...
                COALESCE(AVG(CASE 
                    ...
                    ELSE 0 
                END), 0)::integer AS "nrItensDistancia"
            FROM 
                "public"."TbSensorRegistro" sr
                JOIN "public"."TbSensor" s ON sr."cdSensor" = s."cdSensor"
                JOIN "public"."TbDispositivo" d ON sr."cdDispositivo" = d."cdDispositivo"
